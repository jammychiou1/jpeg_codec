    ...
    while True:
        ...
        if marker in [b'\xff\xd8', b'\xff\xd9']:
            yield marker, b''
                        ...
                        b'\xff\xfe', b'\xff\xdb', b'\xff\xdd', b'\xff\xc0', b'\xff\xc2', b'\xff\xc4']:
            ...
            sz = int.from_bytes(data[:2], 'big')
            payload = data[:sz]
            body, data = entropy_coded(data[2:])
            yield marker, payload
        else:
            return 'error'
...
for segment in parse(data):
    print(segment[0])
    if segment[0] == b'\xff\xc2':
        print(segment[1].hex())
    if segment[0] == b'\xff\xda':
        sz = int.from_bytes(segment[1][:2], 'big')
        print(segment[1][:sz].hex())
